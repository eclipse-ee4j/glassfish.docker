...
```
docker run -p 8080:8080 -p 4848:4848 glassfish:7.0.20
```
...
```
docker run -e SUSPEND=true -p 8080:8080 glassfish:7.0.20 runembedded
```
...
public class WelcomePageITest {
    ...
    @Container
    private final GenericContainer server = new GenericContainer<>("glassfish:7.0.20").withExposedPorts(8080);
